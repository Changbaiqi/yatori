...
//    private static final Boolean IsOpenmail = ConfigUtils.loadingConfig().getSetting().getEmailInform().getEmail()!="";
    ...
    public void study1(){
        ...
        Set<String> includeCourses = user.getCoursesCustom().getIncludeCourses();
        for (String includeCours : includeCourses) {
            //遍历获取必学课程
            RequiredCourseListRequest requiredCourseList = CourseAction.getRequiredCourseList(user, resultList.getCircleId().toString(),includeCours);
