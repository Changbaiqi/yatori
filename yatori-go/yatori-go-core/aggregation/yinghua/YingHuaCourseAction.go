...
// 课程列表
func CourseListAction(cache *yinghua2.YingHuaUserCache) ([]YingHuaCourse, error) {
	var courseList []YingHuaCourse
	listJson := yinghua2.CourseListApi(*cache)
...
// 获取指定课程的信息
func CourseDetailAction(cache *yinghua2.YingHuaUserCache, courseId string) (YingHuaCourse, error) {
	courseDetailJson := yinghua2.CourseDetailApi(*cache, courseId)
	//超时重登检测
...
func VideosListAction(UserCache *yinghua2.YingHuaUserCache, course YingHuaCourse) ([]YingHuaVideo, error) {
	var videoList []YingHuaVideo
	videoSet := make(map[string]int)
	//接口一爬取视屏信息
	listJson := yinghua2.CourseVideListApi(*UserCache, course.Id)
	...
	for i := 1; i < 999; i++ {
		listJson1 := yinghua2.VideWatchRecodeApi(*UserCache, course.Id, i)
...
func ExamDetailAction(UserCache *yinghua2.YingHuaUserCache, nodeId string) ([]YingHuaExam, error) {
	var examList []YingHuaExam
	jsonStr := yinghua2.ExamDetailApi(*UserCache, nodeId)
	//超时重登检测
...
// 开始考试
func StartExamAction(UserCache *yinghua2.YingHuaUserCache, exam YingHuaExam) error {
	//开始考试
	startExam, err := yinghua2.StartExam(*UserCache, exam.CourseId, exam.NodeId, exam.ExamId)
	if err != nil {
