...
func VideosListAction(UserCache *yinghuaApi.YingHuaUserCache, course YingHuaCourse) ([]YingHuaVideo, error) {
...
// 开始考试
func StartExamAction(UserCache *yinghuaApi.YingHuaUserCache, exam YingHuaExam) error {
	//开始考试
	...
	if items, ok := jsonList.([]interface{}); ok {
		for _, item := range items {
			...
			if obj, ok := item.(map[string]interface{}); ok {
				...
				allow, _ := strconv.Atoi(obj["nodeId"].(string))
...
func StartWorkAction(userCache *yinghuaApi.YingHuaUserCache, exam YingHuaWork) error {
	//开始考试
	startWork, err := yinghuaApi.StartWork(*userCache, exam.CourseId, exam.NodeId, exam.WorkId)
	if err != nil {
		log.Print(log.INFO, err.Error())
		return errors.New(err.Error())
	}
	//如果开始考试状态异常则直接抛错
	if gojsonq.New().JSONString(startWork).Find("_code") == 9 {
