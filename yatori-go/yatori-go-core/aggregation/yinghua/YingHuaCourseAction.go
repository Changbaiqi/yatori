...
// 开始考试
func StartExamAction(userCache *yinghuaApi.YingHuaUserCache, exam YingHuaExam, aiType string, apiKey string) error {
	//开始考试
	...
	for k, v := range topic.ExamTopics {
		...
		aiAnswer, err := utils.AggregationAIApi(aiType, aiMessage, apiKey)
		if err != nil {
...
func StartWorkAction(userCache *yinghuaApi.YingHuaUserCache, work YingHuaWork, aiType string, apiKey string) error {
	//开始考试
