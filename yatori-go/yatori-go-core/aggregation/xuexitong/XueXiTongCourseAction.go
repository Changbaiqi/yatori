...
func PullCourseChapterAction(cache *xuexitong.XueXiTUserCache, cpi, key int) (ChaptersList, error) {
	...
	if course, ok := chapterData[0]["course"].(map[string]interface{}); ok {
		if data, ok := course["data"].([]interface{}); ok {
			if len(data) > 0 {
				if knowledge, ok := data[0].(map[string]interface{})["knowledge"].(map[string]interface{})["data"].([]interface{}); ok {
