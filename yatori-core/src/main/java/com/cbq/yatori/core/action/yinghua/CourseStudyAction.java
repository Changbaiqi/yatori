...
public class CourseStudyAction implements Runnable {
    ...
    private final List<Topic> topics = new ArrayList<>(); // 题库
    private final HashMap<String, Integer> topicMd5 = new HashMap<>(); // 题库md5映射
    ...
    public void toStudy() {
        ...
        switch (coursesCustom.getVideoModel()) {
            // 普通模式
            case 0, 1 -> {
//                acoVideo = (long) videoInforms.size();
                if (newThread) {
    ...
    private void study1() {
        ...
        for (NodeList videoInform : videoInforms) {
            // 当视屏没有被锁时
            ...
            if (videoInform.getNodeLock() == 0) {
                ...
                // 获取到视屏观看信息
                VideoInformRequest videMessage;
                ...
                if (videMessage.getResult().getData().getVideoId().isEmpty() && user.getCoursesCustom().getAutoExam() == 1) {
                    ExamInformRequest exam = ExamAction.getExam(user, String.valueOf(videoInform.getId()));
                    // 过滤误判的考试或者视屏
                    if (!exam.getExamInformResult().getList().isEmpty()) {
                ...
                // 视屏总时长
                lookVo(videoInform,videMessage,studyId,cache);
            }
            addAco();
        }
    }
    ...
    private void lookVo(NodeList videoInform, VideoInformRequest videMessage, long studyId, AccountCacheYingHua cache) {
            ...
            try {
                ...
                if (submitStudyTimeRequest != null) {
                    ...
                    if (result != null && result.getData() != null)
                            studyId = result.getData().getStudyId();
    ...
    private void update() {
        ...
        for (VideoList videoList : zList) {
            videoInforms.addAll(videoList.getNodeList());
        }
    }
...
//        String examId = String.valueOf(exam.getExamInformResult().getList().get(0).getId());
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            String answer = "";
            // 如果存有相关的题那么直接获取答案回答
            if (topicMd5.containsKey(turnMd5())) {
        ...
        try {
            log.info("{}:课程:{}考试成功！对应考试试卷{}，服务器信息：{}",
                    user.getAccount(),
                    courseInform.getName(),
                    videoInform.getName(),
                    ConverterStartExam.toJsonString(startExamRequest)
            );
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
        }
...
//        String workId = String.valueOf(exam.getExamInformResult().getList().get(0).getId());
        ...
        if (startExamRequest.getCode() == 9) {
            try {
                log.info("{}:课程:{}课后作业考试失败！对应作业试卷{}，服务器信息：{}",
                        user.getAccount(),
                        courseInform.getName(),
                        videoInform.getName(),
                        ConverterStartExam.toJsonString(startExamRequest)
        ...
        try {
            log.info("{}:课程:{}课后作业考试成功！对应作业试卷{} ,服务器信息: {}",
                    user.getAccount(),
                    courseInform.getName(),
                    videoInform.getName(),
                    ConverterStartExam.toJsonString(startExamRequest)
    ...
    public static class Builder {
        ...
        public CourseStudyAction build() {
            ...
            // 将所有视屏都加入到集合里面
            for (VideoList videoList : zList) {
                courseStudyAction.videoInforms.addAll(videoList.getNodeList());
            }
